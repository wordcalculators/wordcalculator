...
document.addEventListener('DOMContentLoaded', () => {
  // Check if allTools is available before initializing search
  if (typeof allTools === 'undefined') {
    console.error('allTools is not defined. Make sure tools-directory.js is loaded.');
  ...
  const y = document.getElementById('pf-year');
...
const grammarInlineBadge = document.getElementById('grammar-inline-badge');
...
// Directory of tools for Quick Search (uses allTools from tools-directory.js)
// Check if allTools exists, otherwise use empty array
const toolDirectory = typeof allTools !== 'undefined' ? allTools : [];
...
function initMobileMenu(){
  ...
  if (openAuthBtnMobile) openAuthBtnMobile.addEventListener('click', () => {
    close();
    openAuth();
  });
}
...
function initToolQuickSearch(){
  const input = document.getElementById('toolSearchInput');
  const suggestionsBox = document.getElementById('toolSearchSuggestions');
  if (!input || !suggestionsBox) return;

  // Check if toolDirectory is available
  if (!Array.isArray(toolDirectory) || toolDirectory.length === 0) {
    console.warn('toolDirectory is empty or not an array. Search functionality will be limited.');
    
    // Set up basic input without suggestions
  ...
  function renderSuggestions(query){
    ...
    // Filter tools that contain the query anywhere
    const allMatches = toolDirectory.filter(tool =>
      tool.name.toLowerCase().indexOf(value) !== -1
    );
    ...
    // Rank matches: closer / more relevant matches first
    function rankTool(tool){
      const name = tool.name.toLowerCase();
      const pos = name.indexOf(value);
      const starts = name.startsWith(value) ? 0 : 1;
      const lenDiff = Math.abs(name.length - value.length);
      return starts * 100 + pos * 2 + lenDiff;
    }

    currentResults = allMatches.slice().sort((a, b) => {
      return rankTool(a) - rankTool(b);
    });

    currentResults.slice(0, 10).forEach(tool => {
      const li = document.createElement('li');

      const iconEl = document.createElement('i');
      iconEl.className = tool.icon || 'fa-solid fa-wand-magic-sparkles';
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter'){
      ...
      const first = currentResults.length
        ? currentResults[0]
        : toolDirectory.find(tool => tool.name.toLowerCase().indexOf(value) !== -1);
...
function handleSelectionChange(){
  ...
  if (txt && inEditor && sel.rangeCount > 0){
    ...
    detWords.textContent = w;
    detChars.textContent = c;
    detSentences.textContent = s;
...
function updateReadabilityGauge(score, w, s){
  ...
  if (!w || !s || score === null || isNaN(score)){
    ...
    hintEl.textContent =
      'Type at least one full sentence to see where your text sits on the readability scale.';
...
function updateKeywords(wordCount, text){
  ...
  if (wordCount===0 || !text.trim()){
    keywordList.innerHTML =
      '<p class="placeholder">Start typing to see keywords...</p>';
    return;
  }
  ...
  if (!out.length){
    keywordList.innerHTML =
      '<p class="placeholder">No significant keywords found...</p>';
    return;
  }
...
function applyCase(fn){
  ...
  if(sel){
    ...
    setTimeout(handleSelectionChange,80);
  } else {
    editor.textContent=fn(editor.textContent);
    invalidateGrammarState();
    updateFullStats();
    autoGrowEditor();
    persistEditor();
...
function toTitleCase(){
  applyCase(t=>t.replace(/\w\S*/g,s=>s[0].toUpperCase()+s.slice(1).toLowerCase()));
}
function toSentenceCase(){
  applyCase(t=>t.toLowerCase()
    .replace(/(^\s*|[.!?]\s+)(\w)/g,(_,p1,p2)=>p1+p2.toUpperCase()));
}
...
function stripDiacritics(){
  ...
  autoGrowEditor();
  persistEditor();
  notify('Diacritics removed');
}

// ---------- Copy ----------
function copyEditorText(){
  const text = (isSelectionMode ? getSelected() : editor.textContent) || '';
  if (!text.trim()){
    return notify('No text to copy');
  }

  if (navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(text)
      .then(() => notify('Text copied'))
      .catch(() => fallbackCopy());
  } else {
    fallbackCopy();
  }
...
function findReplace(){
  ...
  editor.textContent=(editor.textContent||'').replace(new RegExp(f,'g'),r);
  invalidateGrammarState();
  updateFullStats();
  autoGrowEditor();
  persistEditor();
  notify('Find and replace completed');
}
...
function tokenSentences(t){
  return (t.trim().match(/[^.!?]+[.!?]+/g) ||
    (t.trim()? [t.trim()] : []))
    .map(s=>s.trim());
}
function calcAllReadability(){
  ...
  if (!words.length || !sentences.length){
    alert('Not enough text to calculate.');
    return;
  }
  ...
  const smog = sentences.length >= 3
    ? (1.0430 * Math.sqrt(polysyllables * (30 / sentences.length)) + 3.1291)
    : NaN;
...
async function lookupThesaurus(){
  ...
  if (!word){
    out.innerHTML='<p>Type a word to search.</p>';
    return;
  }
  ...
  try{
    ...
    if (!data.length){
      out.innerHTML='<p>No synonyms found.</p>';
      return;
    }
    out.innerHTML = data.map(d=>
      `<button class="btn" style="margin:4px" onclick="insertSynonym('${d.word.replace(/'/g,"\\'")}')">${d.word}</button>`
    ).join('');
  }catch(e){
...
async function generateStatsPDF(){
  ...
  doc.setFontSize(16);
  doc.text(siteName, 40, 40);
  doc.setFont('helvetica','normal');
  doc.setFontSize(10);
  doc.text(siteUrl, 40, 56);

  doc.setFont('helvetica','bold');
  doc.setFontSize(14);
  doc.text('Word Statistics', 40, 90);
  doc.setFont('helvetica','normal');
  doc.setFontSize(12);
  ...
  let y = 112;
  stats.forEach(s => { doc.text(s, 40, y); y += 18; });

  doc.setFont('helvetica','bold');
  doc.text('Content (excerpt):', 40, y + 12);
  doc.setFont('helvetica','normal');
...
function shareStats(platform){
  ...
  const baseText =
    `I just analyzed ${words} words (${chars} characters) with WordCalculator.net`;
  ...
  switch(platform){
    ...
    case 'email':
      shareUrl =
        `mailto:?subject=${encodeURIComponent('Word count from WordCalculator.net')}`+
        `&body=${text}%0A%0A${urlRaw}`;
      break;
    default:
...
function paintHighlights(plain, matches){
  ...
  for(const m of matches){
    ...
    const tip = escapeHtml(
      `${message}${replacements[0]?` • Suggestion: ${replacements[0].value}`:''}`
    );
    html += escapeHtml(before) +
      `<span class="lt-error" data-tip="${tip}">${escapeHtml(err)}</span>`;
    idx = offset + length;
  }
...
function updateGrammarToolbarUI(){
  if (!grammarBtn || !grammarInlineBadge) return;
  if (grammarEnabled){
    grammarInlineBadge.textContent = 'ON';
    grammarInlineBadge.classList.remove('tb-inline-off');
    grammarInlineBadge.classList.add('tb-inline-on');
    grammarBtn.classList.remove('tb-btn-off');
  } else {
    grammarInlineBadge.textContent = 'OFF';
    grammarInlineBadge.classList.remove('tb-inline-on');
    grammarInlineBadge.classList.add('tb-inline-off');
...
function clearGrammarHighlights(){
  ...
  if (grammarResults){
    grammarResults.innerHTML =
      '<p class="placeholder">Grammar check is currently turned off from the toolbar.</p>';
  }
}

// Toolbar Grammar button click
...
async function checkGrammar(){
  ...
  if (!grammarEnabled){
    if (grammarResults){
      grammarResults.innerHTML =
        '<p class="placeholder">Grammar check is turned off from the toolbar. Turn it ON to analyze your text.</p>';
    }
  ...
  if(!text){
    if (grammarResults){
      grammarResults.innerHTML =
        '<p class="placeholder">Please enter some text to check grammar.</p>';
    }
  ...
  try{
    ...
    if(matches.length===0){
      grammarResults.innerHTML =
        `<div class="issue-card"><div class="issue-head"><div class="issue-type">Grammar</div><div class="issue-badges"><span class="badge badge-err">0 issues</span></div></div><div class="issue-body" style="color:var(--ok)"><i class="fa-solid fa-circle-check"></i> No critical issues found.</div></div>`;
      editor.textContent = text;
      invalidateGrammarState();
      persistEditor();
      return;
    ...
    const cards = matches.slice(0,120).map((m,i)=>{
      ...
      const levelBadge = (m.rule && m.rule.issueType==='misspelling')
        ? 'badge-err'
        : (m.rule && m.rule.issueType==='typographical')
          ? 'badge-warn'
          : 'badge-err';
      const best = (m.replacements && m.replacements[0])
        ? m.replacements[0].value
        : null;
      const excerpt = lastPlainText
        .slice(
          Math.max(0,m.offset-25),
          Math.min(lastPlainText.length,m.offset+m.length+25)
        )
        .replace(/\n/g,' ');
      return `
          ...
          <div class="issue-head">
            ...
            <div class="issue-badges">
              <span class="badge ${levelBadge}">
                ${m.rule && m.rule.issueType ? escapeHtml(m.rule.issueType) : 'issue'}
              </span>
            </div>
          </div>
    ...
    grammarResults.innerHTML =
      cards || '<p class="placeholder">Issues found, but none with automatic fixes.</p>';
    notify(`${matches.length} issues highlighted`);

  }catch(e){
    console.error(e);
    grammarResults.innerHTML =
      '<p class="placeholder">Grammar service unavailable (CORS/network). You can add a tiny backend proxy later for reliability.</p>';
    if (applyAllBtn) applyAllBtn.style.display = 'none';
...
function applyAllFixes(){
  if(!lastPlainText || !lastMatches.length){
    return notify('No fixes available');
  }
...
function initDropdownsAnchored(){
  ...
  document.addEventListener('click', (e)=>{
    const clickedInside = openMenus.some(
  ...
  document.querySelectorAll('.tb-dropdown').forEach((dd)=>{
    ...
    const place = () => {
      ...
      left = Math.max(
        window.scrollX + 8,
        Math.min(left, window.scrollX + window.innerWidth - mw - 8)
      );
...
function textToSpeech(){
  ...
  if(!('speechSynthesis' in window))
    return notify('Text-to-speech not supported in your browser');
  isSpeaking=true;
  if (speakIcon) speakIcon.className = 'fas fa-stop';
  ...
  currentUtterance.onend=()=>{
    isSpeaking=false;
    if (speakIcon) speakIcon.className='fas fa-volume-up';
  ...
  notify(isSelectionMode?'Speaking selected text':'Speaking all text');
}
function stopSpeech(){
  isSpeaking=false;
  if (speakIcon) speakIcon.className='fas fa-volume-up';
...
function initAuthModal(){
  ...
  // Handle mobile redirect for auth buttons
  ...
  // Add click listeners to auth buttons (desktop only)
  const setupAuthButtons = () => {
    const openBtn = document.getElementById('openAuthBtn');
    const openBtnMobile = document.getElementById('openAuthBtnMobile');
    ...
    if (openBtnMobile) {
      const originalClick = openBtnMobile.onclick;
      openBtnMobile.onclick = null;
      openBtnMobile.addEventListener('click', (e) => {
        if (!handleAuthClick(e)) {
          // Desktop behavior - show modal and close mobile menu
...
function fakeLogin(e){
  e.preventDefault();
  notify('Logged in (demo only)');
  document.getElementById('authOverlay').click();
  return false;
}
function fakeRegister(e){
  e.preventDefault();
  notify('Account created (demo only)');
  document.getElementById('authOverlay').click();
  return false;
}
